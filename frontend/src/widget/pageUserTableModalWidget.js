...
function PageUserTableModalWidget(props) {
  ...
  const {
    keyField, onTableChange, rowEvents, onMemModal, setOnMemModal, memClickEvent, initUserId
  } = props;
  ...
  useEffect(() => {
    getModalMemList(searchWrd).then((resp) => {
      setData(resp.data["resultList"]);
    });
  }, []);
  ...
  return (
    <>
      <CModal
        show={onMemModal}
        onClose={() => setOnMemModal(!onMemModal)}
        size="sm"
      >
