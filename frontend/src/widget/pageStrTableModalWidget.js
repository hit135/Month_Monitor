...
import {CButton, CModal, CModalBody, CModalFooter, CModalHeader, CModalTitle, CCol, CSelect, CInput} from "@coreui/react";
...
function PageStrTableModalWidget(props) {
  ...
  const {onStrModal, setOnStrModal, clickStrRow, initStrCode, areaId} = props;
  ...
  return (
    <>
      <CModal show={onStrModal} onClose={() => setOnStrModal(!onStrModal)} /*size="sm"*/ className={"mt-5"}>
        <CModalHeader closeButton>
          <CModalTitle>상점 선택</CModalTitle>
        </CModalHeader>
        <CModalBody className={"strModal"}>
          <CCol sm="5" className={"float-left pl-0"}>
            <CInput placeholder="검색어 입력" id={"searchWrd"}
                    onKeyUp={e => {
          ...
          <BootstrapTable keyField={"strCode"} data={repo.slice()} columns={viewColumns} onTableChange={handleTableChange} rowEvents={rowEvents}
                          remote striped hover condensed
                          pagination={paginationFactory({
                            page: pageItem.page, sizePerPage: pageItem.sizePerPage, totalSize: pageItem.totalElementsCount, paginationSize: 5
                            , showTotal: false, hidePageListOnlyOnePage: true, hideSizePerPage: true})}
