...
function PageAreaTreeModalWidget(props) {
  ...
  return (
    <>
      <CModal show={onAreaModal} onClose={() => setOnAreaModal(!onAreaModal)} size="sm" className={"mt-5 strModal"}>
        ...
        <CModalBody>
          <CCol className={"pl-0"}>
            <CCol sm="8" className={"float-left pl-0"}>
              <CInput placeholder="검색어 입력" onChange={e => setInputSearchValue(e.target.value)} onKeyUp={e => {if (e.key === "Enter") clickSearchTree();}} />
            </CCol>
